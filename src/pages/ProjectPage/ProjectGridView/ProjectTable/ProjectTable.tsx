...
function flattenRows(
  tasks: ProjectTask[],
  expanded: Set<ProjectTask["id"]>,
  depth = 0
): { depth: number; task: ProjectTask }[] {
  return tasks.flatMap((task) =>
    task.children?.length && expanded.has(task.id)
      ? [
          { depth, task },
          ...flattenRows(task.children, expanded, depth + 1),
        ]
      : [{ depth, task }]
  );
}
...
export function ProjectTable({
  ...
  const [columnOrder, setColumnOrder] = useState<ProjectTableColumnKey[]>(() =>
    columns.map(({ key }) => key)
  );
  const [expanded, setExpanded] = useState<Set<ProjectTask["id"]>>(
    () => new Set()
  );
        ...
        const labelKey = (
          {
            ...
            dueDate: "dueDate",
            priority: "priority",
            documentPageLink: "documentPage",
            milestone: "milestone",
          } as Partial<Record<ProjectTableColumnKey, string>>
        )[key];
        return [
          {
            ...column,
  ...
  const minWidth = useMemo(
    () =>
      orderedColumns.reduce((sum, column) => sum + column.minWidth, 0),
    [orderedColumns]
  );
  const rows = useMemo(() => flattenRows(tasks, expanded), [expanded, tasks]);
  const toggleExpansion = (taskId: ProjectTask["id"]) =>
    setExpanded((current) => {
          ...
          {rows.map(({ depth, task }) => (
            <TableRow
              className={`hover:bg-accent hover:text-accent-foreground ${
                backgrounds[Math.min(depth, backgrounds.length - 1)]
              }`}
              key={task.id}
            >
              {orderedColumns.map((column) => (
                <ProjectTableCell columnKey={column.key} key={column.key}>
                  {column.key === "isFinished" ? (
                    <FinishedCell
                    ...
                    <StatusCell bucketNames={bucketNames} task={task} />
                  ) : column.key === "priority" ? (
...
function ProjectTableCell({
  ...
  children: ReactNode;
  columnKey: ProjectTableColumnKey;
}) {
  const overflow =
    columnKey === "subject" || columnKey === "taskKey"
      ? "overflow-hidden"
      : "";
  return (
    <TableCell
      className={`h-[32px] min-w-0 px-[10px] py-[4px] ${overflow}`}
    >
      {children}
    </TableCell>
  );
