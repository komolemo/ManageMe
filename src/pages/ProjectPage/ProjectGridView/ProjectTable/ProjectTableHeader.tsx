...
export const ProjectTableHeader = memo(function ProjectTableHeader({ columns, onColumnOrderChange }: Props) {
  ...
  const clearDrag = () => { setDragged(null); setDragOver(null); setPosition("before") };
  const startDrag = (event: DragEvent<HTMLTableCellElement>, key: ProjectTableColumnKey) => {
    setDragged(key);
    event.dataTransfer.effectAllowed = "move";
    event.dataTransfer.setData("text/plain", key);
  };
  const dragColumn = (event: DragEvent<HTMLTableCellElement>, key: ProjectTableColumnKey) => {
  ...
  const dropColumn = (event: DragEvent<HTMLTableCellElement>, target: ProjectTableColumnKey) => {
    event.preventDefault();
    const source = dragged ?? event.dataTransfer.getData("text/plain") as ProjectTableColumnKey;
  ...
  return (
    <TableHeader className="sticky top-0 z-20 bg-muted text-xs font-semibold text-muted-foreground">
      <TableRow className="hover:bg-muted">
        {columns.map((column) => {
          const isOver = dragOver === column.key && dragged !== column.key;
          return (
            <TableHead
              aria-label={t("project.moveColumn", { columnName: column.label })}
              className={`h-auto cursor-grab select-none border-l-2 border-r-2 px-[8px] py-[4px] text-muted-foreground transition-colors active:cursor-grabbing ${isOver ? `${position === "after" ? "border-r-primary" : "border-l-primary"} bg-accent text-accent-foreground` : "border-l-transparent border-r-transparent"} ${dragged === column.key ? "opacity-50" : ""}`}
              draggable
              key={column.key}
              onDragEnd={clearDrag}
            ...
            >
              {column.hideHeaderText ? <span className="sr-only">{column.label}</span> : column.label}
            </TableHead>
          );
        })}
      </TableRow>
